...
module.exports = (config) => {
    ...
    const getNodes = function (ids) {
        const d = when.defer();
        const filter = {
            id: {$in: ids}
        };
        if (this instanceof Message) {
            logger.debug(this.clone('what', 'db.find()', 'filter', {}));
        }
    ...
    storage.editNode = (
        ...
        .then((rights) => {
            ...
            .then(() => {
                ...
                if (relIds.length > 0) {
                    return getNodes.call(msg, relIds.concat(user))
                    .then((nodes) => {
